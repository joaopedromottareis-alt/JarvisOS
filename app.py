...
st.markdown("""
    ...
    /* ==================== CAIXA DO CALENDÁRIO FIXA (Garante Visibilidade) ==================== */
    .calendar-wrapper-box {
        background-color: #0a0a0a !important;
        border-radius: 34px !important; /* Estilo do seu SCSS */
        padding: 20px !important;
        border: 1px solid rgba(212, 175, 55, 0.2) !important;
        box-shadow: 0 20px 40px rgba(0,0,0,0.6) !important;
        min-height: 520px !important; /* Força espaço para renderização do iframe */
    }
...
if st.session_state.autenticado and username:
    ...
    # 4. ABA CRONOGRAMA OPERACIONAL (CALENDÁRIO SEGURO)
    with aba_calendario:
        ...
        # Variáveis definidas antecipadamente de forma segura
        dia_num_hoje = datetime.date.today().strftime("%d")
        dias_traduzidos = {
            "Monday": "SEGUNDA-FEIRA", "Tuesday": "TERÇA-FEIRA", "Wednesday": "QUARTA-FEIRA",
            "Thursday": "QUINTA-FEIRA", "Friday": "SEXTA-FEIRA", "Saturday": "SÁBADO", "Sunday": "DOMINGO"
        }
        dia_name_hoje = dias_traduzidos.get(datetime.date.today().strftime("%A"), "HOJE")
        ...
        with col_esq_info:
            ...
            with st.expander("➕ CRIAR EVENTO LOCAL", expanded=False):
                ...
                data_ev = st.date_input("Data do evento:", datetime.date.today())
                h_ini = st.time_input("Início da atividade:", datetime.time(9, 0))
                
                if st.button("Agendar Evento"):
                    if nome_ev:
                        start_dt = datetime.datetime.combine(data_ev, h_ini)
                        id_unico = f"manual_{int(time.time())}"
                        db["eventos_locais"].append({
                            "id": id_unico, "title": f"{h_ini.strftime('%H:%M')} - {nome_ev}", 
                            "start": start_dt.isoformat(), "allDay": False
                        })
                        salvar_dados(db); st.rerun()

        with col_dir_cal:
            # Lista base estruturada para renderizar os compromissos
            lista_render = [{
                "title": "🎬 Turno Iniciado",
                "start": datetime.datetime.combine(datetime.date.today(), datetime.time(7, 0)).isoformat(),
                "allDay": False
            }]
            if db.get("eventos_locais"):
                lista_render.extend(db["eventos_locais"])

            # Renderização encapsulada garantindo altura mínima
            st.markdown('<div class="calendar-wrapper-box">', unsafe_allow_html=True)
            calendar(
                events=lista_render, 
                options={
                ...
                key=f"jarvis_calendar_{st.session_state.cal_version}"
            )
