...
# ==================== SISTEMA DE PROTEÇÃO E AUTENTICAÇÃO DE LOGIN ====================
credentials = {
    "usernames": {
        "admin": {
            "name": "Senhor",
            "password": "$2b$12$Mco6XwCH79S452R2gB5PFeIes3G8z9q/XkW9b5iV1zYlWv8iL1PDe" # Senha: admin123
        }
    }
}
...
# ==================== BLOCO DO ECOSSISTEMA JARVIS OS (APENAS SE LOGADO) ====================
elif authentication_status:
    authenticator.logout('Sair do Sistema', 'sidebar')
    st.sidebar.markdown(f"<div style='color:#8e8e9a; font-size:13px;'>Operador Conectado:<br><b style='color:#d4af37;'>{name}</b></div>", unsafe_allow_html=True)
    ...
    with aba_calendario:
        ...
        if service:
            st.markdown("<span style='color: #2ecc71; font-size: 13px; font-weight:500;'>🟢 Sincronização em tempo real com Google Agenda activa.</span>", unsafe_allow_html=True)
        else:
        ...
        st.markdown("<br>", unsafe_allow_html=True)
        with st.container(border=True):
            # VISÃO EM LINHA DO TEMPO CONFIGURADA NATIVAMENTE (Abre direto em listDay como na foto)
            options = {
                "initialView": "listDay",
                "headerToolbar": {
